...
class CpuBackend:
    ...
    def run(self, spec: BenchmarkSpec, dataset: DatasetLayout, *, time_budget_seconds: float, force_rebuild: bool = False) -> BackendResult:
        ...
        command = [
            ...
            "--workers", ",".join(str(v) for v in worker_candidates),
            "--tile-sizes", ",".join(str(v) for v in tile_candidates),
        ...
        config = {
            "workers": int(metrics["actual_workers"]), "requested_workers": int(metrics["requested_workers"]),
