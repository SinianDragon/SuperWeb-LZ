/*
 * CPU Conv2D Benchmark Runner for Windows
 *
 * Key optimizations vs original:
 *   1. Weight transposed to [K,K,Cin,Cout] for contiguous oc access
...
// Compute layout: [K, K, Cin, slice_Cout]   (contiguous in Cout → auto-vectorize inner oc loop)
...
// ─── Conv2D Kernel ────────────────────────────────────────────────────────────
// Optimized loop order: for each spatial point, iterate kh→kw→ic (read input once),
// then broadcast to all oc (contiguous weight_t access, auto-vectorizable).

void compute_conv2d_slice(const float* __restrict input,
                          const float* __restrict weight_t,   // transposed: [K,K,Cin,slice_Cout]
                          float* __restrict output,
                          int h, int w, int c_in,
                          int slice_c_out, int k, int pad,
                          int out_h, int out_w,
            ...
            for (int kh = 0; kh < k; ++kh) {
                ...
                for (int kw = 0; kw < k; ++kw) {
                    ...
                    for (int ic = 0; ic < c_in; ++ic) {
                        ...
                        // → compiler can auto-vectorize with SSE2/AVX2
                        for (int oc = 0; oc < slice_c_out; ++oc) {
...
void run_multithreaded(const float* input, const float* weight_t, float* output,
                       int h, int w, int c_in, int slice_c_out, int k, int pad,
    ...
    for (int t = 0; t < num_workers; ++t) {
        ...
        threads.emplace_back(compute_conv2d_slice,
            input, weight_t, output, h, w, c_in, slice_c_out, k, pad,
...
int main(int argc, char** argv) {
    ...
    int h = 0, w = 0, c_in = 0, total_c_out = 0, k = 0, pad = 0;
