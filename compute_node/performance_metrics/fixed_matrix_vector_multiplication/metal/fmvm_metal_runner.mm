...
struct ConvParams {
    ...
    uint32_t k; uint32_t pad; uint32_t out_h; uint32_t out_w;
};
...
int main(int argc, char** argv) {
    @autoreleasepool {
        ...
        uint32_t out_h = h + 2 * pad - k + 1;
        uint32_t out_w = w + 2 * pad - k + 1;
        ...
        ConvParams params = {(uint32_t)h, (uint32_t)w, (uint32_t)c_in, (uint32_t)c_out, (uint32_t)k, (uint32_t)pad, out_h, out_w};
